...
work_dir <- 'C:/Users/haachicanoy/Documents/GitHub/interdependence_circos/_macro_to_region'
...
lapply(1:length(umeas),function(i)
{
  ...
  originDF <- data.frame(origin, order.regions.col)
  destinyDF <- data.frame(destiny, order.regions.row)
  
  originDF <- originDF[order(originDF$order.regions.col),]
  rownames(originDF) <- 1:nrow(originDF)
  levels.origin.regions <- as.character(originDF$origin)
  
  destinyDF <- destinyDF[order(destinyDF$order.regions.row),]
