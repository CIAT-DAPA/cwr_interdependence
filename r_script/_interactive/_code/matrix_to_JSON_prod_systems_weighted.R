# Matrix to JSON format using R: Extract top 5 of important crops by flow - Production systems
# H. Achicanoy & C. Khoury
# CIAT, 2016

# Loading information
work_dir <-'C:/Users/haachicanoy/Documents/GitHub/interdependence_circos'
all_elements <- read.csv(paste(work_dir, '/_production_systems/_circos/Prod/prod_sum_items_region.csv', sep=''))
####
meas <- list.dirs(paste(work_dir,'/_production_systems/_interdependence_analysis/_outputs',sep=''),full.names=F,recursive=F)
meas <- gsub(pattern='by_',replacement='',meas)
region_crops <- lapply(1:length(meas),function(i)
{
  z <- read.csv(paste(work_dir, '/_production_systems/_interdependence_analysis/_outputs/by_',meas[i],'/prod_countries_regions_all_merge.csv',sep=''))
  return(z)
})
...
write.csv(top5_element, paste(work_dir, '/_interactive/_useful_info/most_important_prod_crop_flows_weighted.csv', sep=''), row.names=FALSE)
...
work_dir <- 'C:/Users/haachicanoy/Documents/GitHub/interdependence_circos'
flows <- list.files(path=paste(work_dir, '/_interactive/_flows_matrix/prod_without_hierarchical_level', sep=''), pattern='*_weighted.csv$', full.names=TRUE)
flows <- flows[c(2,1,3)]
...
top5_info <- read.csv(paste(work_dir, '/_interactive/_useful_info/most_important_prod_crop_flows_weighted.csv', sep=''))
...
names(matrices) <- c('production_quantity','harvested_area','production_value')
...
metrs <- c('tonnes', 'ha', 'million US$') # c('production_quantity','harvested_area','production_value')
...
sink(paste(work_dir, '/_interactive/_json/prod_interdependence_complex_text_weighted.json', sep='')) # redirect console output to a file
