...
pr_data_elements <- pr_data_elements[,c('Item','Country','Element', 'Region', paste('Y',2009:2011,sep=''))]
pr_data_elements <- unique(pr_data_elements)
rownames(pr_data_elements) <- 1:nrow(pr_data_elements)

library(dplyr)
pr_sum_region_data_elements <- lapply(2009:2011,function(i)
{
  library(dplyr)
  eval(parse(text=paste('pr_sum_region_data_elements <- as.data.frame(dplyr::summarise(group_by(pr_data_elements, Item, Region, Element), sum(Y',i,')))',sep='')))
})
pr_sum_region_data_elements2 <- merge(pr_sum_region_data_elements[[1]],pr_sum_region_data_elements[[2]],by=c("Item","Region","Element"))
pr_sum_region_data_elements3 <- merge(pr_sum_region_data_elements2, pr_sum_region_data_elements[[3]],by=c("Item","Region","Element"))
names(pr_sum_region_data_elements3)[ncol(pr_sum_region_data_elements3)-2:0] <- paste("Y",2009:2011,sep="")
pr_sum_region_data_elements <- pr_sum_region_data_elements3
rm(pr_sum_region_data_elements2, pr_sum_region_data_elements3)

pr_sum_region_data_elements$Average <- rowMeans(pr_sum_region_data_elements[,c("Y2009","Y2010","Y2011")])

write.csv(pr_sum_region_data_elements, paste(work_dir, '/_circos/Prod/prod_sum_items_region.csv', sep=''), row.names=FALSE)

region_crops <- lapply(1:length(meas),function(i)
{
  z <- read.csv(paste(work_dir, '/_interdependence_analysis/_outputs/by_',meas[i],'/prod_countries_regions_all_merge.csv',sep=''))
  return(z)
})
region_crops <- Reduce(function(...) rbind(..., deparse.level=1), region_crops)
region_crops <- region_crops[,c('Item','Region_crops')]
region_crops <- unique(region_crops)

pr_sum_region_data_elements <- merge(pr_sum_region_data_elements, region_crops, by='Item')
pr_sum_region_data_elements <- unique(pr_sum_region_data_elements); rownames(pr_sum_region_data_elements) <- 1:nrow(pr_sum_region_data_elements)

library(dplyr)
pr_sum_regioncrops_data_elements <- as.data.frame(dplyr::summarise(group_by(pr_sum_region_data_elements, Region_crops, Region, Element), sum(Average)))
colnames(pr_sum_regioncrops_data_elements)[4] <- 'Average'

write.csv(pr_sum_regioncrops_data_elements, paste(work_dir, "/_circos/Prod/regions_sourceofprod_weighted.csv", sep=''), row.names=FALSE)
...
cOrgData_region <- read.csv(paste(work_dir, "/_circos/Prod/regions_sourceofprod_weighted.csv", sep=''))
